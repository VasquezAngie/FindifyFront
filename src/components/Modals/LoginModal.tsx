  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showToast(response.status, 'Error', data.message || 'Error en la autenticación');
        setIsLoading(false);
        return;
      }
      ...
      console.log('Usuario autenticado con token:', token, 'Login success:', loginSuccess);
    ...
    <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50" onClick={handleOverlayClick}>
      <div className="bg-white rounded-xl shadow-xl w-96">
        <div className="p-6 border-b flex items-center">
          <h2 className="text-xl font-bold text-center flex-grow">Iniciar Sesión</h2>
          <button 
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4 flex items-center space-x-2">
              <input
                ...
                id="rememberMe"
              />
              <label htmlFor="rememberMe" className="select-none">Recordarme</label>
            </div>
            <button
              type="submit"
